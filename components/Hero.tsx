...
import { ArrowRight, Calendar, MapPin, Users, Plane, Loader2, ShieldCheck, Info, ChevronDown, ChevronUp, Sparkles, ArrowLeftRight } from 'lucide-react';
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="pt-2">
                   <div className={`rounded-2xl transition-all duration-300 border ${formData.includeInsurance ? 'bg-silver-50 dark:bg-zinc-900/50 border-silver-200 dark:border-zinc-800 p-4' : 'border-transparent'}`}>
                       ...
                       <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showInsuranceDetails ? 'max-h-64 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
                           <div className="text-xs text-silver-500 space-y-2 pl-9 border-l-2 border-silver-200 dark:border-zinc-800 ml-3">
                               <p className="flex items-center gap-2">
                                 <span className="w-1 h-1 rounded-full bg-green-500"></span> 
                                 Medical coverage up to <strong>$100,000</strong>
                               </p>
                               <p className="flex items-center gap-2">
                                 <span className="w-1 h-1 rounded-full bg-green-500"></span> 
                                 Trip Cancellation up to <strong>100%</strong> of cost
                               </p>
                               <p className="flex items-center gap-2">
                                 <span className="w-1 h-1 rounded-full bg-green-500"></span> 
                                 Baggage loss/damage up to <strong>$2,500</strong>
                               </p>
                               <p className="flex items-center gap-2">
                                 <span className="w-1 h-1 rounded-full bg-green-500"></span> 
                                 Trip Delay reimbursement up to <strong>$500/day</strong>
                               </p>
                               <p className="flex items-center gap-2">
                                 <span className="w-1 h-1 rounded-full bg-green-500"></span> 
                ...
                >
                  ...
                  {isSearching ? <Loader2 className="animate-spin" /> : (
                    <>
                      Find Flights <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                    </>
                  )}
                </button>
